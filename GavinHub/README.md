...
GitHub 的“保存连接”只保存 Token 与 Gist ID，不上传或下载数据；“同步数据”会明确报告本次是上传、下载、合并还是没有变化。首次同步会先查找并恢复账号中已有的 GavinHub Gist，确认基线后才允许上传本地修改。若发现多个备份，会停止写入并要求明确填写 Gist ID。
...
- 输入 `arxiv`、`sc` 或 `gs` 后按 Tab 可进入学术搜索模式；也可直接输入 `arxiv 关键词`、`sc 关键词`
- 点击右上角齿轮可切换搜索引擎与壁纸来源
...
- 启动只保留统一显现与搜索聚焦两段，壁纸异步解码且重型效果在开场动画后计算
